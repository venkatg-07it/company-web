...
@Component({
  selector: 'app-non-laser-print',
  templateUrl: './non-laser-print.component.html',
  styleUrls: ['./non-laser-print.component.css']
})
export class NonLaserPrintComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    let fieldInfoUrl = AppConstants.FILE_PATH_UPLOAD +
      AppConstants.FILE_PATH_NON_LASER_PRINT +
  ...
  loadForm() {
    ...
    let url = APIConstants.MISCELLANEOUS_API + "/OtherthanLaserList/" + loadDay
    this.showLoader = true;
    this.success = false;
    this.error = false;
  ...
  print() {
    ...
    let url = APIConstants.MISCELLANEOUS_API + "/printNonLaserList"
    ...
    let content = `
      <div>
        ...
        <div class="centerEleDiv1">
          <span><b>LOADDAY-${loadDay} </b></span>&nbsp;&nbsp;<span><b>QTY-${qty}</b></span>   
        </div>
